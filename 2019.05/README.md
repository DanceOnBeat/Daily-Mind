...
* 阻塞IO：用户进程发起IO调用，内核不会直接返回，而是等待数据准备并从内核拷贝至进程之后才会解锁进程，CPU需等待IO
* 非阻塞IO：用户进程发起IO调用，内核会直接返回当前数据是否处理完毕，不会block用户进程。当处理完毕后，进程需要主动调用read去获取。虽然进程没被block，但是进程也需要轮询去检查文件描述符状态，普通的轮询浪费CPU资源
* 多路复用IO：有select/poll/epoll等实现，发起调用时，内核会直接返回，进程被select或是其他方法阻塞。但一个进程可以管控多个文件描述符，当其中一个准备完成，即可返回。在处理大量IO请求时比多线程/阻塞IO有优势。select在监控的文件描述符的数量上有限制，epoll对其进行优化
* 异步IO：用户进程调用之后，操作系统直接返回。当数据准备完成，内核会给进程发送信号。如Linux的AIO

#### Node实现异步IO
Node中是通过线程池来模拟异步IO的，JS单线程处理任务，线程池来发起IO等调用，配合事件循环来执行回调。在Windows操作系统中采用内置的IOCP来管理线程池，而Linux下需要自行维护线程池，配合epoll尽量降低CPU的使用率。Linux下虽然有原生提供的异步IO方式，AIO，但是其仅支持内核IO中的O_DIRECT方式读取，导致无法利用系统缓存，因此没有使用。

* * *
### 5.21
事件驱动模型和多线程模型：

* 多线程模型是一个请求对应一个线程，在高并发场景下，内存资源会很快耗尽
* 事件驱动模型只需要一个额外的线程来模拟异步IO，多个请求通过epoll管理。某个IO操作完成后，会在事件循环中执行回调。因此避免了频繁的上下文切换，也避免了多线程消耗内存的问题，适用于高并发场景
